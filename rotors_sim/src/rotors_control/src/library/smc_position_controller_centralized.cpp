...
void SmcPositionController::CalculateThrust(Eigen::Vector3d* thrust) const {
  ...
  if(1)
  {
    ...
    double rho_p = hatKp[0] + hatKp[1]*xp.norm() + hatKp[2]*pow(xp.norm(),2);
    ...
    Eigen::Vector3d tau_p = -lam_p*sp - delTau_p + Eigen::Vector3d(0,0,0);
    // Eigen::Vector3d tau_p = -lam_p*sp + Eigen::Vector3d(0,0,0);
    *thrust = tau_p;
