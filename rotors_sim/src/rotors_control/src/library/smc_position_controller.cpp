...
void SmcPositionController::InitializeParameters() {
  ...
  controller_parameters_.Fp_ = controller_parameters_.payload_/2;
  ...
  controller_parameters_.inertia_(0,0) = (2*ml2 + ms2/2 + md2/2);
  controller_parameters_.inertia_(1,1) = (2*ml2 + ms2/2 + md2/2);
  controller_parameters_.inertia_(2,2) = (4*ml2 + ms2/2);
...
void SmcPositionController::ComputeThrust(Eigen::Vector3d* thrust) const {
  ...
  Eigen::Vector3d kp = (controller_parameters_.eeta_p_)*Eigen::Vector3d(1,1,0) + 
                        (controller_parameters_.Fp_ + controller_parameters_.eeta_p_)*(controller_parameters_.gravity_ * e_3 + 
                          command_trajectory_.acceleration_W - 
                          controller_parameters_.theta_p_.cwiseProduct(velocity_error));
